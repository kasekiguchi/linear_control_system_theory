...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
\frametitle{4.1 状態方程式}
%-------------------------------------------------------------
離散システムとは
\begin{itemize}
	\item 連続システムを離散化してから制御系を設計する場合
	\item もともと離散システムで表されるシステム\\
	（計算機内での計算アルゴリズム，経済システムなど）
\end{itemize}
を制御する場合に用いられる．

離散システムの解析設計手法は，連続時間システムの場合と\Blue{ほとんど同様に議論できる}ので，
ここでは連続時間システムに対する議論との対比を行い，
連続時間システムの場合と異なる部分について述べる．

\vspace{5mm}

以下の状態方程式で表されている離散システムを扱うものとする．
\begin{align}
...
\end{align}
となり，\\
座標変換による$A$, $B$, $C$, $D$行列の変化は
\Blue{連続時間システムの場合と一致する}．
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
\frametitle{4.3 伝達関数と状態方程式 \\
4.4 可制御性，可観測性}
%-------------------------------------------------------------
\Blue{伝達関数と状態方程式}\\
連続時間システムを伝達関数表現するためにはシステムの\Blue{ラプラス変換}を用いた．\\
離散時間システムに対しても同様に\Blue{$z$変換}というものがあり，伝達関数表現することがある．\\
しかし，ここでは直接コントローラの設計に用いないので説明は省略する．

\vspace{5mm}

\Blue{可制御性，可観測性}\\
連続時間システムと離散時間システムの状態の座標変換に対する
状態方程式の各行列の変換は厳密に一致している．\\
よって厳密さを求めなければ連続時間システムで行った議論がそのまま離散時間システムにも成り立つ．
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
\frametitle{4.5 システムの応答}
%-------------------------------------------------------------
状態方程式
\begin{align}
 x_{k+1} &= A x_k + B u_k \tag{\ref{d_state_eq}}
\end{align}
より明らかに
\begin{align*}
 x_1 &= A x_0 + B u_0 \\
...
\end{align}
を満たすように選べると仮定する
% (ジョルダン形式になる場合も後の議論はほとんど同じになる)
．ここで$\lambda_i$は行列$A$の固有値である．この
$\Lambda$を用いれば
\begin{align}
...
\end{equation}
であることが分かる．\\
つまり，\Blue{行列$A$の固有値の絶対値が1未満であればシステムは安定である}ことが分かる．
\end{frame}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
\frametitle{4.7 状態フィードバック}
%-------------------------------------------------------------
\Blue{連続時間システムとまったく同様}に考えることができる．\\
いま状態$x_k$が利用できるとする．このとき入力を
\begin{equation}
 u_k = F x_k + v_k
\end{equation}
とすれば状態方程式は
...
\end{equation}
となる．ここで$v_k \in \mathbb{R}^m $は新しい入力である．
このように状態
フィードバックされた\Blue{閉ループ系の安定性は$(A + BF)$の固有値によって決まる}．\\
これは連続時間システムの場合とまったく同じである．
状態フィードバックによりシステムを安定化したり，システムの極を変えたりすることができる．
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
\frametitle{4.7 状態フィードバック(極配置問題)}
%-------------------------------------------------------------
\Blue{離散時間システムの極配置問題}:
$(A+BF)$の固有値を任意の与えられた値にする問題(連続時間システムの場合と同じ)．\\

\vspace{5mm}

連続時間システムと離散時間システムの座標変換がまったく同じ形で表されることに注意すれば，
離散時間システムに対する極配置問題は連続時間システムと同一の手法で解けることがわかる．
ただし，ここではシステムの安定性を保証するために指定する極は安定極
\Blue{(絶対値が1未満)}である必要がある．
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
\frametitle{4.7 状態フィードバック(最適制御)}
%-------------------------------------------------------------
...
が安定になる．
一般に$Q$と$R$の選び方は試行錯誤となるが，選び方の指針は\Blue{連続時間システム
と同じ}である．
\end{frame}
...
この積分器
\begin{align}
 \eta_{k+1} = \eta_k + (r_k - y_k) = \eta_k + (r_k - C x_k) 
\end{align}
とシステム
\begin{align}
   x_{k+1} &= A x_k + B u_k \tag{\ref{d_st_proper_state_eq}} \\
       y_k &= C x_k \tag{\ref{d_st_proper_output_eq}}
\end{align}
を合わしたシステム(拡大系)は
\begin{align}
 \left(\begin{array}{c}
...
せる状態フィードバックは極配置法や最適制御法で求めることができる．
\end{frame}
...
\frametitle{4.7 状態フィードバック(サーボ系)}
%-------------------------------------------------------------
$r_k$が一定値$r$であるとすると，システムは安定であるから
$i\rightarrow \infty $で\\
拡大システムの状態はそれぞれ$x_i \to x_\infty , \eta_i \to \eta_\infty$\\
\begin{itemize}
	\item $x_i \to x_\infty$ から出力$y$も$y_{\infty} = C x_{\infty}$に収束．
	\item $\eta_i \to \eta_\infty$から
		\begin{equation}
...
%\end{align}
%の状態$x_k$
%を知ることができる．このようなシステムを\Blue{状態観測器(オブザーバ)}と呼ぶ．
%$(A + KC)$を安定にする$K$は$(C, A)$が可観測であるときに存在する．実際の求め
%方としては
%\begin{equation}
% \xi_{k+1} = A^T \xi_k + C^T u_k
%\end{equation}
%なるシステムを安定化させるフィードバック
%\begin{equation}
% u_k =  K^T \xi_k
%\end{equation}
%を求めれば$A^T + C^T K^T$が安定になるので$A + K C$も安定となる．
%\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[t]
\frametitle{4.8 出力フィードバック\\
(状態観測器によるフィードバック系の安定性)}
%-------------------------------------------------------------
離散時間システムにおいて状態観測器を用いた場合のフィードバック系の安定性は，
連続時間システムの場合と同様の議論により，
全体の系の極が状態フィードバックを施したときのシステムの極($(A+BF)$の固有値)と状態観測器の極($(A + KC)$の固有値)で表わされることが分かる．
\end{frame}
