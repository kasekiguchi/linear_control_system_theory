...
\begin{figure}[htbp]
 \begin{center}
  	...
  	\includegraphics[width=6cm]{figures/discretize.png} % scale = 100%
...
\textcolor{blue}{離散化されたシステムの状態方程式と出力方程式}は
\begin{align}
 x_{k+1} &= e^{A_cT} x_k 
...
\begin{align}
 ...
 &=
  ...
  S^{-1}
\end{align}
となり，\textcolor{blue}{\bf 離散化されたシステムの$A$行列の固有値は$e^{\lambda_{ci} T}$}となる．
\end{frame}
...
\begin{align}
 ...
 &=
  ...
  S^{-1} \tag{3.7}
\end{align}
\textcolor{blue}{\bf 離散化されたシステムの$A$行列の固有値は$e^{\lambda_{ci} T}$}となる．
よって\\
\hspace*{0.5cm}
連続時間システムの\textcolor{blue}{$A_c$行列の固有値の実部が負である}\\
\hspace*{0.5cm}
$\Longrightarrow$ 離散化されたシステムの\textcolor{blue}{$A$行列の固有値の絶対値は1未満となる}．\\
...
\end{equation}
で近似すれば\textcolor{blue}{状態観測器を用いることなく}状態フィードバックをすることが可能．\\
...
サンプリングインターバルが長いと，\textcolor{red}{連続時間コントローラと同等の性能が得られない}場合がある．

\hspace*{2cm}
$\Downarrow$ 

何らかの意味でディジタルコントローラの性能が連続時間コントローラの性能に近付くように，コントローラの離散化を工夫する\textcolor{blue}{ディジタル再設計の問題} を考える必要がある．
